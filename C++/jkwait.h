...
#define JK_SET_LIMIT(rate)       jk::set_limit(rate);
#define JK_TRY_WAIT              jk::try_wait();
#define JK_WAIT_STATUS           jk::wait_status();
...
namespace jk {
	...
	*  3、调用库函数可能的不确定耗时：如 正则匹配，子字符串查找, json, protobuf, xml等等数据解析及构造。。。。。等等
	*  4、较长的函数进入和离开都可以加下
	*
